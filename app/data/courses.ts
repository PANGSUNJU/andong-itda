 ...
 * 걷는 길 — 자체 큐레이션 코스
 *
 * 공공 API에 없는 데이터다. **확인하고 없다**(2026-08-16 전수 실측 → ADR-008):
 * 두루누비 `courseList`는 전국 144코스가 전부이고 그것이 곧 「코리아둘레길」이다.
 * 해안선과 접경을 도는 길이라 내륙인 안동은 지날 수가 없고, 경북에 잡히는 것은
 * 경주·포항·영덕·울진 넷뿐으로 전부 동해안이다. TourAPI 여행코스(contentTypeId=25)는
 * 차량 이동을 전제한다. 그래서 "버스 없이 걸어서 이어지는 길"은 직접 만든다.
 *
