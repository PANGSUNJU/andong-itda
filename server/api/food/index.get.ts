 ...
 *
 * ⚠️ 여기만 아직 옛 지역 코드로 조회한다. 법정동 코드로 옮기면 15건 → 27건이 된다.
 *    관광지 쪽(ADR-034)과 함께 옮기지 않은 것은 늘어나는 12곳의 향토음식 태깅을
 *    확인해야 하기 때문이다. → KOR_SERVICE_REGION
 *
 * 캐시는 /api/spots와 같은 1일이다. 상류가 월 단위로 움직이는 데이터라
 * 더 짧게 볼 이유가 없다. 15건이라 캐시가 비어도 첫 요청이 2~3초면 끝난다.
 */
export default defineCachedEventHandler(
  async (): Promise<FoodPlace[]> => {
     ...
     * 15곳 전부를 재검색해 한 건도 더 못 찾았다 — 이미지 보충이 0/7이었던 것과
