...
/**
 * 지도가 이보다 좁게 확대되지는 않는 폭(m)
 *
 * 정류장 다섯 곳이 300m 안에 모여 있으면 그 300m에 딱 맞춰 확대된다. 건물 단위라
 * 어느 동네인지가 화면에서 사라진다. 최소 폭을 둬서 동 이름과 큰길이 함께 남게 한다.
 *
 * ⚠️ `MapCard`에 있던 상수를 여기로 올렸다. 지도만 쓰는 수가 아니다 — 관광지 상세가
 *    "내 위치 점을 찍어도 화면에 들어오나"를 이 수로 판단한다. 두 곳이 각자 들고
 *    있으면 지도는 넓어졌는데 점은 여전히 안 찍히는 식으로 조용히 어긋난다.
 *    실측(2026-09-15): 중앙신시장 상세에서 300m는 보이고 400m는 화면 밖이었다.
 */
export const MAP_MIN_SPAN_M = 600

/**
 * 걸어서 갈 만한 거리(m)
 *
 * 홈이 관광지를 "걸어서 갈 수 있는 곳"과 "버스로 갈 수 있는 곳"으로 가르는 선이고,
 * 목적지를 정했을 때 **걸어가서 탈 수 있는 승강장**을 고르는 선이기도 하다.
 *
 * ⚠️ `index.vue`와 `SpotCard.vue`가 각자 들고 있던 2000을 여기로 올렸다.
 *    MAP_MIN_SPAN_M을 올린 것과 같은 이유다 — 한쪽만 고치면 화면은 "걸어서 갈 수
 *    있는 곳"이라 부르는데 승강장은 안 끌어오는 식으로 조용히 어긋난다.
 *
 * ⚠️ 목적지행 승강장을 끌어올 때 이 선이 **없으면** 도보 311분짜리 승강장이 1순위
 *    칩으로 올라온다. 실측(2026-09-16): 도산서원에서 병산서원을 목적지로 잡으면
 *    가장 가까운 승차 정류장이 20.8km였다. 그건 안내가 아니다. → ADR-050
 */
export const WALKABLE_M = 2000
