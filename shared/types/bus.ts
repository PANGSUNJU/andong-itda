...
/**
 * 운행 상태 — 도착정보와 운행 차량 수를 결합한 판정 결과
 *
 * 화면 문구가 여기서 갈린다.
 *   arriving  → "12분 후"
 *   waiting   → "접근 중인 버스가 없어요. 배차가 길어요."
 *   routeIdle → "이 노선은 지금 다니지 않아요." (다른 노선은 다닌다)
 *   offHours  → "지금은 버스가 다니지 않는 시간이에요." (경고)
 *
 * ⚠️ 예전에는 셋이었고 마지막이 `closed`("오늘 운행이 끝났어요")였다. 그 이름이
 *    **새벽에 거짓말을 했다.** 실측(2026-09-10 06:36): 안동 전체 58개 노선 63대가
 *    운행 중인데 월영교의 112번만 0대였다. 첫차가 08:25이기 때문이다. 그 시각은
 *    끝난 것이 아니라 아직 시작 전인데 화면은 "끝났어요"라고 적었다.
 *
 *    `runTotCnt`가 0인 이유는 셋이다 — 첫차 전 · 막차 후 · 이 노선만 미운행.
 *    노선 하나만 보면 셋을 못 가른다. **안동 전체에 운행 중인 차량이 있는가**가
 *    빠져 있던 신호다. 그리고 상태 이름은 시각을 주장하지 않는다("다니지 않는
 *    시간"). 상류가 첫차·막차를 주지 않으므로(ADR-016) 시계로 추정할 근거가 없다.
 *    → ADR-036, ADR-015의 연장
 */
export type ServiceStatus = 'arriving' | 'waiting' | 'routeIdle' | 'offHours'
